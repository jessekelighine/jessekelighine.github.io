...
\author{\url{jessekelighine@gmail.com}}
\date{\today}
%<--------------------------------------------------------------------------->%
...
\begin{align}\label{eq:fisher}
	\mathcal{I}_\theta
	:= \E{\left(\frac{\partial}{\partial\theta}\log f(x\given\theta)\right)^2
		\given[\middle]\theta}
\end{align}
...
That is, a random variable $X$ is an might contain a certain amount of
...
We use $\frac{\partial}{\partial\theta}f(x\given\theta)$ since we want 
to measure the ``percentage of change'' due to theta.
Then it is squared since we want to measure the ``change'',
or the ``variance.''
It can be shown that the expectation of $\frac{\partial}{\partial\theta}f(x\given\theta)$
is zero.

Explicitly, let $\ell_{x}(\theta):=\log f(x\given\theta)$ and
...
\begin{align}\label{eq:mle}
	\hat{\theta}_{\text{MLE}}
	\asim
	\normal{0,\frac{1}{\mathcal{I}_{\theta}}},
\end{align}
...
\begin{proof}
	...
	\begin{align*}
		\E{\ddot\ell_{x}(\theta)}
		= \E{ \frac{\ddot f(x\given\theta)}{f(x\given\theta)}
		- \left(\frac{\dot f(x\given\theta)}{f(x\given\theta)}\right)^2 }
		&= \E{\frac{\ddot f(x\given\theta)}{f(x\given\theta)}}
		- n\mathcal{I}_{\theta} \\
		&= \int_{\mathcal{X}}
