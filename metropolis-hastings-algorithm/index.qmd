...
## Demonstration: Sample from a Unnormalized Density Function
...
```{r}
totally_arbitrary_constant <- 69420
dtriangle2_unnormalized <- function(x) totally_arbitrary_constant * dtriangle2(x)
```
...
1. We plot against the true density function `dtriangle2` for comparison.
...
First, we set up the true parameters and the prior parameters of the problem:
...
1. We bind `data` and `prior_params` to the function directly.
   This is because a conditional distribution sampler should simple take the current state of the Markov chain
...
1. An note on `gibbs_sampling`'s implementation:
   it does not take `data` or `prior_params` as input arguments.
   ...
   `sample_posterior_mu` and `sample_posterior_sigma2`
   to handle `data` and `prior_params`.
